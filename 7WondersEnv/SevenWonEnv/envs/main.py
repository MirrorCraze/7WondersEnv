...
TransitionWithoutReward = namedtuple('TransitionWithoutReward',
...
if __name__ == '__main__':
    ...
    env = gym.make("SevenWonderEnv-v0")
    # set up matplotlib
    ...
    def select_action(state,possibleAction):
        ...
        EpsGreed_threshold = EpsGreed_END + (EpsGreed_START - EpsGreed_END) * \
                             math.exp(-1. * steps / EpsGreed_DECAY)
        steps += 1
    ...
    for i_episode in range(prevEp, num_episodes):
        if i_episode %200 == 0:
            ...
            torch.save(tarNet.state_dict(),os.path.join("ModelDict","tarNet" + str(i_episode) + ".pt"))
        ...
        for t in count():
            ...
            if done:
                ...
                for i in range(subMemLen):
                    mem = subMemory[i]
                    memory.push(mem[0],mem[1],mem[2],(DISCOUNTED_RATE**(subMemLen-i))*reward)
